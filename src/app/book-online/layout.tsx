  ...
  return (
    <BookingProvider>
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
                ...
                {steps.slice(0, -1).map((step, i) => (
                  <div key={step.path} className="flex items-center flex-1 last:flex-none">
                    <div className="flex flex-col items-center">
                      ...
                      <span className={`text-[10px] sm:text-xs mt-1 hidden sm:block ${i <= currentIdx ? "text-primary font-semibold" : "text-gray-400"}`}>
                        {step.label}
                      </span>
                    </div>
                    {i < steps.length - 2 && (
                      <div className={`flex-1 h-0.5 mx-1 sm:mx-2 mt-0 sm:mt-[-4px] transition-all duration-300 ${i < currentIdx ? "bg-primary" : "bg-gray-200"}`} />
                    )}
                  </div>
                ))}
              </div>
        ...
        <div className="max-w-3xl mx-auto px-3 sm:px-6 lg:px-8 py-6 sm:py-10 lg:py-16">
          {children}
        </div>
      </div>
    </BookingProvider>
